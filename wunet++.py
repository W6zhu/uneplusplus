...
for i, (train_index, test_index) in enumerate(kf.split(sliced_image_dataset, sliced_mask_dataset)):
    ...
    checkpoint = ModelCheckpoint(f'C:/Users/Mittal/Desktop/wunet++/best_model{i}.keras', monitor='val_loss', save_best_only=True)
    ...
    plt.xlabel('Epoch')
    plt.tight_layout()
    plt.savefig(f'C:/Users/Mittal/Desktop/wunet++/process{i}.png')
    plt.close()

    max_dice_coef = max(history.history['dice_coef'])
    max_val_dice_coef = max(history.history['val_dice_coef'])

    f = open("C:/Users/Mittal/Desktop/wunet++/output.txt", "a")
    print("max dice: ", max_dice_coef, file=f)
    print("max val dice: ", max_val_dice_coef, file=f)
    f.close()
    
    model.load_weights(f'C:/Users/Mittal/Desktop/wunet++/best_model{i}.keras')
    ...
    for z in range(5):
        ...
        plt.savefig(f'C:/Users/Mittal/Desktop/wunet++/predict/fold{i}_{z}.png')
        plt.close()
