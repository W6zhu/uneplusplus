...
def generate_slices(image_directory, mask_directory):
    sliced_image_dataset = []
    sliced_mask_dataset = []

    all_image_files = os.listdir(image_directory)
    all_image_files = sorted([f for f in all_image_files if f.endswith('.nii')])
    ...
    for image_name in all_image_files:
        ...
        # Load mask
        mask_path = os.path.join(mask_directory, image_name)  # Assuming mask filenames match image filenames
        ...
        for slice_index in range(image_data.shape[2]):
            sliced_image_dataset.append(image_data[:, :, slice_index])
            sliced_mask_dataset.append(mask_data[:, :, slice_index])

    # Ensure datasets are numpy arrays and have correct shape
    sliced_image_dataset = np.expand_dims(np.array(sliced_image_dataset), axis=-1).astype('float64')
    sliced_mask_dataset = np.expand_dims(np.array(sliced_mask_dataset), axis=-1).astype('float64')

    return sliced_image_dataset, sliced_mask_dataset
...
all_image_files = os.listdir(image_directory)
# Ensure we are only dealing with .nii files and sort them to match images with masks
all_image_files = sorted([f for f in all_image_files if f.endswith('.nii')])

selected_image_files = random.sample(all_image_files, 10)  # Select 10 random files
...
#     sliced_image_filenames.append(image_name.split('.')[0])
...
for i, (train_index, test_index) in enumerate(kf.split(sliced_image_dataset, sliced_mask_dataset)):
    ...
    sliced_image_filenames = [os.path.splitext(image_name)[0] for image_name in selected_image_files if image_name.endswith('.nii')]
